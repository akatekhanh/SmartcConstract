...
TAG = 'FCFS'
class Transaction(IconScoreBase):
    _ADDRESS_ARRAY = "ADDRESS_ARRAY"
    _TIMESTAMP_ARRAY = "TIMESTAMP_ARRAY"
    ...
    def __init__(self, db: IconScoreDatabase) -> None:
        ...
        self._timestamp_array = ArrayDB(self._TIMESTAMP_ARRAY, db, value_type=int)
    ...
    def apply(self):
        ...
        elif len(self._address_array) < self._max.get():
            if self.msg.sender is not None:
                self._address_array.put(self.msg.sender)
                self._timestamp_array.put(self.tx.timestamp)
                json_result = {}
